...
export default function AdminStudentView() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-8 sm:p-10 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row items-center gap-8">
        ...
        <div className="text-center sm:text-left flex-1">
          ...
          <div className="flex flex-wrap justify-center sm:justify-start gap-3 mt-6">
            <span className="px-4 py-1.5 bg-brand/10 text-brand rounded-lg text-xs font-bold uppercase tracking-widest">Student</span>
      ...
      {/* Tabs */}
      <div className="flex bg-white p-1.5 rounded-xl border border-slate-100 shadow-sm w-fit mx-auto sm:mx-0">
        <button 
          onClick={() => setActiveTab('profile')}
          className={`px-8 sm:px-12 py-3 rounded-xl font-bold text-sm transition-all ${
            activeTab === 'profile' ? 'bg-brand text-white shadow-sm shadow-brand/5' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          Profile Information
        </button>
        <button 
          ...
          className={`px-8 sm:px-12 py-3 rounded-xl font-bold text-sm transition-all ${
            activeTab === 'interest' ? 'bg-brand text-white shadow-sm shadow-brand/5' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          Career Interest
        </button>
        <button 
          ...
          className={`px-8 sm:px-12 py-3 rounded-xl font-bold text-sm transition-all ${
            activeTab === 'tracker' ? 'bg-brand text-white shadow-sm shadow-brand/5' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
          Tracker
        </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="relative h-64 rounded-2xl overflow-hidden shadow-sm">
                  ...
                  <div className="absolute bottom-6 left-6">
                    <span className="px-4 py-1.5 bg-brand text-white rounded-lg text-xs font-bold uppercase tracking-widest">Recommended</span>
                  </div>
                </div>
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl">
                      <img src="https://picsum.photos/seed/mentor/100/100" className="w-14 h-14 rounded-xl object-cover" alt="Mentor" />
                      <div>
            ...
            <div className="space-y-12">
              {[
                ...
                <div key={i} className="flex gap-6 relative">
                  ...
                  <div className={`w-14 h-14 ${item.bg} ${item.color} rounded-xl flex items-center justify-center shrink-0 z-10`}>
